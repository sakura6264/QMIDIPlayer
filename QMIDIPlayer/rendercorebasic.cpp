...
/* one implement of "IRenderCore"
 * Color is red.
 */
#define notew 7
#define noteh 3
void RenderCores::RenderCoreBasic::init(SDL_Renderer *ren, SDL_Window *screen,
                                        QMidiOut *midiout) {
  this->screen = screen;
  this->ren = ren;
...
void RenderCores::RenderCoreBasic::render(
  ...
  for (int pos = 0;
    ...
    for (RenderEvent evt : rendertask[time + pos]) {
      Uint32 color =
          SDL_MapRGBA(surface->format, evt.vol * 2, 0, evt.voice * 2, 255);
...
RenderCores::RenderCoreBasic::~RenderCoreBasic() {
  if (inited) {
